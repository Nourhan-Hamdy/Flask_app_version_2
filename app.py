...
def light_whrs_guide(Art_Light_Whrs,flag):
	...
	if Art_Light_Whrs >2:
		if flag == False:
			return "Decrease Artificial lighting working hours"
		else:
			return "تقلل من عدد ساعات تشغيل الإضاءة الصناعية"

def fan_whrs_guide(Fan_Whrs,flag):
	if Fan_Whrs == 3:
		if flag == False:
			return "Decrease fan working hours"
		else:
			return "تقلل من عدد ساعات تشغيل المروحة"

def Ac_whrs_guide(AC_Whrs,flag):
	if AC_Whrs == 5 or AC_Whrs == 6 or AC_Whrs == 7:
		if flag == False:
			return "Decrease AC working hours"
		else:
			return "تقلل من عدد ساعات التكييف"

def heater_whrs_guide(Heater_Whrs,flag):
	if Heater_Whrs == 5 or Heater_Whrs == 6 or Heater_Whrs == 7:
		if flag == False:
			return "Decrease heater working hours"
		else:
			return "تقلل من عدد ساعات تشغيل السخان"

def motor_whrs_guide(motor_Whrs,flag):
	if motor_Whrs == 2 or motor_Whrs == 3:
		if flag == False:
			return "Decrease motor working hours"
		else:
			return "تقلل من عدد ساعات تشغيل الموتور"

def kettle_whrs_guide(Kettle_Whrs,flag):
	if Kettle_Whrs == 2 or Kettle_Whrs == 3:
		if flag == False:
			return "Decrease kettle working hours"
		else:
			return "تقلل من عدد ساعات تشغيل الغلاية"
...
def household_predict():
	if request.method == 'POST':
		...
		cons_sent = ""
		if note_results:
			cons_sent = "For energy conservation you should:"
		...
		return render_template('household.html',House_Summ_final_result="Predicted segment during summer season is:"+summ_categ+" "+"segment; with minimum consumption is:"+summ_mini_cons+" "+"pound"+" "+"and maximum consumption is:"+summ_maxi_cons+" "+"pound",
			House_Win_final_result="Predicted segment during winter season is:"+win_categ+" "+"segment; with minimum consumption is:"+win_mini_cons+" "+"pound"+" "+"and maximum consumption is:"+win_maxi_cons+" "+"pound",cons_sent=cons_sent,note_results=note_results)
...
def arab_household_predict():
	if request.method == 'POST':
		...
		cons_sent = ""
		if note_results:
			cons_sent = ":لترشيد استهلاك الكهرباء يُمكن أن"
		...
		# 	House_Win_final_result[1],note_results=note_results)
		return render_template('household-rtl.html',House_Summ_final_result="الشريحة المتوقعة خلال الصيف"+": "+summ_categ+" "+"بأقل استهلاك"+" "+summ_mini_cons+" "+"جنيه"+" "+"وأعلى استهلاك"+" "+summ_maxi_cons+" "+"جنيه",
			House_Win_final_result="الشريحة المتوقعة خلال الشتاء"+": "+win_categ+" "+"بأقل استهلاك"+" "+win_mini_cons+" "+"جنيه"+" "+"وأعلى استهلاك"+" "+win_maxi_cons+" "+"جنيه",cons_sent=cons_sent,note_results=note_results)
...
def community_predict():
	if request.method == 'POST':
		...
		Percent_of_pop_more_60 = 100 - (Percent_of_pop_less_15+Percent_of_pop_bet_15_to_less_60)
		...
		total_min_kwh = float(mini_kwh) * float(tot_pop) * float(0.001)
		total_max_kwh = float(maxi_kwh) * float(tot_pop) * float(0.001)
		return render_template('community.html',Comm_Summ_final_result="Average predicted segment during summer season among household is:"+summ_categ+" "+"segment; with minimum consumption is:"+summ_mini_cons+" "+"pound"+" "+"and maximum consumption is:"+summ_maxi_cons+" "+"pound",
			Comm_Win_final_result="Average predicted segment during winter season among household is:"+win_categ+" "+"segment; with minimum consumption is:"+win_mini_cons+" "+"pound"+" "+"and maximum consumption is:"+win_maxi_cons+" "+"pound",
			mini_kwh="Total minimum power required for the targeted community is about: "+ str(total_min_kwh) + "MWh",maxi_kwh= "Total maximum power required for the targeted community is about: "+ str(total_max_kwh)+"MWh")
...
def arab_community_predict():
	if request.method == 'POST':
		...
		return render_template('community-rtl.html',Comm_Summ_final_result="الشريحة المتوقعة خلال الصيف:"+" "+summ_categ+" "+"بأقل استهلاك"+" "+summ_mini_cons+" "+"جنيه"+" "+"وأعلى استهلاك"+" "+summ_maxi_cons+" "+"جنيه",
			Comm_Win_final_result="الشريحة المتوقعة خلال الشتاء:"+" "+win_categ+" "+"بأقل استهلاك"+win_mini_cons+" "+"جنيه"+" "+"وأعلى استهلاك"+win_maxi_cons+" "+"جنيه", mini_kwh="الحد الأدنى لإجمالي الطاقة بناءً على عدد السكان حوالي: "+str(total_min_kwh)+"  م.و.س",maxi_kwh="الحد الأقصى لإجمالي الطاقة بناءً على عدد السكان حوالي: "+str(total_max_kwh)+"  م.و.س")
